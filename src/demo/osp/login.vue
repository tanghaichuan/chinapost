      ...
      <div class="card-block pt-0 pb-4">
        <t-form ref="form" :model="formData" :rules="rules">
          <t-form-item prop="username">
            <t-input v-model="formData.username" :autofocus="true" icon="account-outline" :placeholder="$t('demo.osp.login.input_username')">
            </t-input>
          </t-form-item>
          <t-form-item prop="password">
            <t-input type="password" icon="lock-outline" v-model="formData.password" :placeholder="$t('demo.osp.login.input_password')">
            </t-input>
          </t-form-item>
          <t-form-item prop="verify">
            <div class="d-flex">
              <div>
                <t-input v-model="formData.verify" :placeholder="$t('demo.osp.login.verify_code')">
                </t-input>
              </div>
              <div class="ml-2">
          ...
          <t-form-item>
            <t-checkbox :label="$t('demo.osp.login.stay_signed_in')" v-model="formData.rememberMe"></t-checkbox>
          </t-form-item>
          <t-form-item class="mb-0">
            <t-button type="primary" @click="submit" block>{{ $t('demo.osp.login.login') }}</t-button>
          </t-form-item>
        </t-form>
      </div>
    </div>
    <p class="d-flex text-center mt-3">
      <a href="#" class="mr-3">{{ $t('demo.osp.login.forgot_password') }}</a>
      <a href="#">用户注册</a>
    </p>
  </div>
        ...
        this.$refs.form.validate(passed => {
          if (passed) {
            this.doLogin(this.formData).then(() => {
              ...
              this.$router.push({ path: '/osp/portal' })
            }).catch(err => {
